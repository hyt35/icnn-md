...
checkpoint_path = '/local/scratch/public/hyt35/ICNN-MD/ICNN-ARTIFICIAL/checkpoints/svm_sum/5000'
...
class_lossfn = torch.nn.BCEWithLogitsLoss(reduction='sum')
...
for idx, (data, target) in enumerate(train_dataloader):
    ...
    target1 = torch.zeros(data1.shape[0])
    target2 = torch.ones(data2.shape[0])
    ...
    def classif_acc(wb):
        ...
        correct += pred.eq(targ).sum(0) # correct, of size (bsize) so we have layerwise
    ...
    fig_loss.suptitle("Test binary cross-entropy loss")
    ...
    break
fig_loss.savefig('svm_log')
fig_prob.savefig('svm_acc_mean')
