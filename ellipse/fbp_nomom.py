...
logging.basicConfig(filename=os.path.join(workdir,datetime.now().strftime("%d-%m_%H-%M-fbp_nomom.log")), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
