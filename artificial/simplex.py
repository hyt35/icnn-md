...
checkpoint_path = '/local/scratch/public/hyt35/ICNN-MD/ICNN-ARTIFICIAL/checkpoints/simplex/'
logging.basicConfig(filename=datetime.now().strftime('logs/%d-%m_%H:%M-simplex.log'), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
...
for epoch in range(n_epochs):
    ...
    x = torch.Tensor([1/3,1/3,1/3]).repeat(bsize,1).to(device) # initialization (bsize, 2)
    
    b = torch.rand(bsize,3).to(device) # hypercube [0,1]^3 uniform
    b_simplex = simplexProjection(b)
    # _b = expSampler.sample()
    # _b = b/b.sum(1)[:,None] # uniform samples on 
    # b = b.to(device)
    ...
    for stepsize in icnn_couple.stepsize:
        ...
        fwd = simplexProjection(icnn_couple.bwd_model(icnn_couple.fwd_model(fwd) - stepsize*fwdGrad))
        loss += obj(fwd)
