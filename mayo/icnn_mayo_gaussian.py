...
# python icnn_mayo_gaussian.py --num_epochs=1500 --num_batches=10 --checkpoint_freq=25
import numpy as np
import torch
...
logging.basicConfig(filename=datetime.now().strftime('logs/%d-%m_%H:%M-mayo-adaptive-cyclicpad.log'), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
    ...
    if args.train:
        ...
        for epoch in range(n_epochs):
            ...
            for idx, batch in enumerate(train_dataloader):
                ...
                batch_noisy_ = batch_true + noise_level*torch.randn_like(batch_true)
            ...
            if (epoch%args.checkpoint_freq == args.checkpoint_freq-1):
                torch.save(icnn_couple.state_dict(), '/local/scratch/public/hyt35/ICNN-MD/ICNN-STL10/checkpoints/Apr12_gaussian/'+str(epoch+1))
            # Log
