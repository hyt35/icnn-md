...
checkpoint_path = 'checkpoints/lsq/'
...
icnn = MDNet_act(dim=2, stepsize_init=0.1, num_iters = 10, stepsize_clamp = (0.05,0.5))
...
if __name__ == "__main__":
    if args.train:
        logging.basicConfig(filename=datetime.now().strftime('logs/%d-%m_%H%M-lsq.log'), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
        ...
        def recon_err_grad(x):
            return autograd.grad(recon_err(x), x)[0]
        stepsize_scales = [1/10,1/4,1/2,1]
