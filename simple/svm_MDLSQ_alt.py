...
checkpoint_path = 'checkpoints/svm_mdlsq_alt/'
log_path = 'logs/%d-%m_%H%M-svm-mdlsq-alt.log'
...
class MDNet_lsq(nn.Module):
    def __init__(self, dim = 10, stepsize_init = 0.01, num_iters = 10, stepsize_clamp = (0.001,0.1)):
        ...
        self.ssmin = stepsize_clamp[0]
        self.ssmax = stepsize_clamp[1]
        self.A = nn.Parameter((0.5*torch.eye(dim)+torch.diag(torch.rand(dim))+0.01*torch.randn(dim,dim))).to(device)
...
icnn = MDNet_lsq(dim=dim, stepsize_init=0.1, num_iters = 10, stepsize_clamp = (0.05,0.5))
#icnn = MDNet_lsq(stepsize_init=0.1, num_iters = 10, stepsize_clamp = (0.05,0.5))
n_epochs = args.num_epochs
# noise_level = 0.05
# reg_param = 0.3
stepsize = 0.1
dataset_size = 1000 # number of random class pair to extractw
...
if __name__ == "__main__":
    if args.train:
        ...
        for idx, (data, target) in enumerate(train_dataloader):
            ...
            ax_acc.plot(mdacc, label = "md adaptive", color='b', marker='^')
            ...
            fig_loss.savefig(fig_dir+'lsq_loss')
            fig_loss.savefig(fig_dir+'lsq_loss.svg')
            ...
            #ax_prob.legend(loc='lower right', fontsize='x-small')
            fig_acc.savefig(fig_dir+'lsq_acc')
            fig_acc.savefig(fig_dir+'lsq_acc.svg')
            #plt.show()
            ...
            # fig.savefig(fig_dir+'lsq_fwd')
            # fig.savefig(fig_dir+'lsq_fwd.svg')
            # plt.show()
            print(icnn.stepsize)
            print(icnn.A)
            print(torch.eig(icnn.A))
            # print(icnn.w)