...
IMPL = 'astra_cuda'
device = 'cuda:0'
workdir = "/local/scratch/public/hyt35/icnn-md-mom/ellipse/deconv_nomom"
...
logging.basicConfig(filename=os.path.join(workdir,datetime.now().strftime("%d-%m_%H-%M-deconv_nomom.log")), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
    ...
    if args.train:
        ...
        for epoch in range(n_epochs):
            ...
            for idx, (batch_, gt_) in enumerate(train_dataloader):
                # add gaussian noise
                batch = gt_.to(device)
                batch_conv = blur_op(batch) + noise_level*torch.randn_like(batch) # blur and 5% gaussian noise
                gt = gt_.to(device)
                ...
                fwd = iterates[-1]
                #loss = recon_err(fwd)
                closeness1 = icnn_couple.fwdbwdloss(batch_conv)
                ...
                err.backward()
                if grad_clip >= 0:
                    torch.nn.utils.clip_grad_norm_(icnn_couple.parameters(), max_norm=grad_clip)
                opt.step()
