...
checkpoint_path = '/local/scratch/public/hyt35/ICNN-MD/ICNN-ARTIFICIAL/checkpoints/simplex/8400'
...
if PLOTLOSS:
    ...
    # min ||Wx-b||_2^2
    def obj(x):
        # return some convex functions parameterized by 
    ...
    for ss in icnn_couple.stepsize:
        ...
        fwd = simplexProjection(icnn_couple.bwd_model(icnn_couple.fwd_model(fwd) - ss*fwdGrad0))
        #fwd = icnn_bwd(icnn_fwd(fwd))
        #fwd_ = fwd.cpu().detach().numpy()
        #print("MD ", svm_loss(fwd).item())
        mdloss.append(obj(fwd).item())
        print(obj(fwd))
    ...
    for stepsize_scale, ms in zip(stepsize_scales, itertools.cycle('>^+*x')):
        ...
        for i in range(20):
            fwd.requires_grad_()
            fwdGrad0 = obj_grad(fwd).detach().clone()
            fwd = simplexProjection(icnn_couple.bwd_model(icnn_couple.fwd_model(fwd) - stepsize*stepsize_scale*fwdGrad0).detach())
            #fwd = icnn_bwd(icnn_fwd(fwd))
            fwd_ = fwd.cpu().detach().numpy()
    ...
    legend1 = ax_loss.legend(handles=[md_adap_patch,md_patch,gd_adap_patch], bbox_to_anchor=(1.05, 0.7),
    ...
    fig_loss.savefig('figs/simp')
    fig_loss.savefig('figs/simp.svg')
