...
# python icnn_mayo_fbp_parallel.py --num_epochs=1500 --num_batches=10 --checkpoint_freq=25
import numpy as np
import torch
...
logging.basicConfig(filename=datetime.now().strftime('logs/%d-%m_%H:%M-mayo-fbp-parallel.log'), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
...
icnn_couple = ICNNCouple(stepsize_init = 0.01, num_iters = 6, stepsize_clamp = (0.001,0.1))
icnn_couple.init_fwd(num_in_channels=1, num_filters=50, kernel_dim=3, num_layers=3, strong_convexity = 0.1)
icnn_couple.fwd_model.initialize_weights()
icnn_couple.init_bwd(num_in_channels=1, num_filters=60, kernel_dim=3, num_layers=4, strong_convexity = 0.5)
    ...
    if args.train:
        ...
        for epoch in range(n_epochs):
            ...
            for idx, batch in enumerate(train_dataloader):
                ...
                projection = fwd_op(batch_true)
                batch_noisy_ = fbp_op(projection + noise_level*torch.randn(projection.size())) # add noise
            ...
            if (epoch%args.checkpoint_freq == args.checkpoint_freq-1):
                torch.save(icnn_couple.state_dict(), '/local/scratch/public/hyt35/ICNN-MD/ICNN-STL10/checkpoints/Apr12_fbp_parallel/'+str(epoch+1))
            # Log
