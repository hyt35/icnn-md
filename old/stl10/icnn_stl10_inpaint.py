...
device = 'cuda'
checkpoint_path = '/local/scratch/public/hyt35/ICNN-MD/ICNN-STL10/checkpoints/Apr20_inpainting/'
logging.basicConfig(filename=datetime.now().strftime('logs/%d-%m_%H:%M-inpainting.log'), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
        ...
        for epoch in range(n_epochs):
            ...
            for idx, (batch_, _) in enumerate(train_dataloader):
                ...
                for stepsize in icnn_couple.stepsize:
                    ...
                    loss += recon_err(fwd)
                #loss = recon_err(fwd)
                closeness1 = icnn_couple.fwdbwdloss(batch_masked)
                closeness2 = icnn_couple.fwdbwdloss(fwd)

                err = loss+(closeness1+closeness2)*closeness_reg
                ...
                total_loss += err.item()
                total_fwdbwd += closeness1.item() + closeness2.item()
                batch_loss += err.item()
                batch_fwdbwd += closeness1.item() + closeness2.item()
                if(idx % args.num_batches == args.num_batches-1):
                    ...
                    print("loss", loss.item(), "closeness inpaint", closeness1.item(), "closeness true", closeness2.item())
