...
device = 'cuda'
checkpoint_path = '/local/scratch/public/hyt35/ICNN-MD/ICNN-STL10/checkpoints/Apr27_denoising/920'
...
mask_ = torch.load('mask_80.pt') # mask with 70% remove
...
if __name__ == '__main__': 
    ...
    for idx, (batch_, _) in enumerate(test_dataloader):
        ...
        for _ in range(10): # Extension
            #fwd = fwd.clamp(0,1)
            fwd.requires_grad_()
            fwdGrad0 = recon_err_grad(fwd).detach().clone()
            fwd = icnn_couple.bwd_model(icnn_couple.fwd_model(fwd) - avgss*fwdGrad0)
            #fwd = icnn_bwd(icnn_fwd(fwd))
            fwd_ = fwd.cpu().detach().numpy()
            print("MD recon", recon_err(fwd).item())
            mdloss.append(recon_err(fwd).item())
        ...
        for stepsize_scale in stepsize_scales:
            ...
            ## ADAM
            adamloss = []
            tmp =  batch_noisy.requires_grad_()
        ...
        #plt.ylim((min_mdloss/2,max_mdloss*5))
        #plt.ylim((min_mdloss/5,min_mdloss*10))
        
        break
    plt.savefig('denoise_log')
    print(icnn_couple.stepsize)
#ss: [0.1,0.0616,0.0401,0.0273,0.0199,0.0148,0.0113,0.0084,0.0060,0.0038]