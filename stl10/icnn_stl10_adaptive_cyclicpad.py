...
logging.basicConfig(filename=datetime.now().strftime('logs/%d-%m_%H:%M-adaptive-cyclicpad.log'), filemode='w', format='%(name)s - %(levelname)s - %(message)s', level=logging.DEBUG)
    ...
    if args.train:
        ...
        for epoch in range(n_epochs):
            ...
            if (epoch%args.checkpoint_freq == args.checkpoint_freq-1):
                torch.save(icnn_couple.state_dict(), '/local/scratch/public/hyt35/ICNN-MD/ICNN-STL10/checkpoints/Apr6_cyclic_adaptive/'+str(epoch+1))
            # Log
